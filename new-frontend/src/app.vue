    ...
    v-resizer(
      ...
      template(v-slot:left)
        #summary-wrapper
          ...
          .timestamp-footer
            span Generated by&nbsp;
            a(v-bind:href='getSpecificCommitLink()', target='_blank')
              strong this version
            span &nbsp;of&nbsp;
            a(v-bind:href='getRepoSenseHomeLink()', target='_blank')
              strong RepoSense
            span &nbsp;(
            a(v-bind:href='getUserGuideLink()', target='_blank')
              strong User Guide
              ...
              #tab-empty(v-else)
                .title
                  ...
                  p
                    | See the &nbsp;
                    a(
                      v-bind:href='getUsingReportsUserGuideLink()',
                      target='_blank'
                    )
                      strong User Guide
...
export default {
  ...
  watch: {
    ...
    '$store.state.tabAuthorshipInfo': function () {
      this.tabInfo.tabAuthorship = Object.assign(
        {},
  ...
  methods: {
    ...
    updateReportZip(evt) {
      ...
      JSZip.loadAsync(evt.target.files[0])
        .then(
          ...
          () => {
            window.alert(
              'Either the .zip file is corrupted, or you uploaded a .zip file that is not generated ' +
                'by RepoSense.'
            );
          }
        )
      ...
      await window.api
        ...
        .then((names) => {
          ...
          this.$store.commit(
            ...
            loadingResourcesMessage
          );

          return Promise.all(names.map((name) => window.api.loadCommits(name)));
        })
        .then(() => {
    ...
    renderAuthorShipTabHash(minDate, maxDate) {
      ...
      const tabInfoLength = Object.values(info).filter((x) => x !== null)
        .length;
    ...
    renderZoomTabHash() {
      ...
      const tabInfoLength = Object.values(zoomInfo).filter((x) => x !== null)
        .length;
